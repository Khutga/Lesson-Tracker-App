...
class OgrenciDetayPenceresi(QWidget):
    ...
    def gecmis_yukle(self):
        ...
        for row in veriler:
            ...
            # İşlem tipini kullanıcı dostu göster
            islem_gosterim = row[2]
            if row[2] == "DersEkle":
                islem_gosterim = "Ders Ekleme"
            elif row[2] == "Odeme":
                islem_gosterim = "Ödeme"

            item = QTreeWidgetItem(self.tree_gecmis, [str(row[0]), row[1], islem_gosterim, row[3], tutar_str])
